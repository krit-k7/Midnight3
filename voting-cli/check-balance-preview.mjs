...
const { PreviewConfig } = await import('./dist/config.js');
...
console.log('Listening to PREVIEW wallet state updates...');
wallet.state().subscribe((state) => {
  const bal = state.unshielded.balances[unshieldedToken().raw] ?? 0n;
  const dustBal = state.dust.balance(new Date());
  console.log(`[PREVIEW STATE] isSynced=${state.isSynced} | unshieldedBal=${bal} | dustBal=${dustBal} | coins=${state.unshielded.availableCoins.length}`);
});
