...
import { type VotingProviders, type DeployedVotingContract } from './common-types';
...
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║              Midnight Anonymous Voting                       ║
║              ─────────────────────                           ║
║              Cast a private, unlinkable ballot on-chain      ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
`;
...
/** Build the voting actions menu, showing current DUST balance in the header. */
const votingMenu = (dustBalance: string) => `
${DIVIDER}
  Voting Actions${dustBalance ? `                      DUST: ${dustBalance}` : ''}
${DIVIDER}
  [1] Cast a YES vote
  [2] Cast a NO vote
  [3] Display current tally
  [4] Exit
${'─'.repeat(62)}
> `;
...
const joinContract = async (providers: VotingProviders, rli: Interface): Promise<DeployedVotingContract> => {
...
const deployOrJoin = async (
  ...
  rli: Interface,
): Promise<DeployedVotingContract | null> => {
  while (true) {
 ...
 * cast a vote (yes/no) or display the current public tally.
 */
const mainLoop = async (providers: VotingProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  const votingContract = await deployOrJoin(providers, walletCtx, rli);
  ...
  while (true) {
    ...
    switch (choice.trim()) {
      case '1':
      case '2': {
        const voteChoice = choice.trim() === '1';
        try {
          await api.withStatus(`Casting ${voteChoice ? 'YES' : 'NO'} vote`, () => api.vote(votingContract, voteChoice));
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
      ...
      case '3': {
        const { tally } = await api.displayVotingTally(providers, votingContract);
        if (tally !== null) {
