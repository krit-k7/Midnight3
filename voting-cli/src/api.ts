...
import { Voting, type VotingPrivateState, createVotingPrivateState, witnesses } from '@shadowballot/voting-contract';
...
// Pre-compile the voting contract with ZK circuit assets
const votingCompiledContract = CompiledContract.make('voting', Voting.Contract).pipe(
...
/** Snapshot of the public voting tallies read from the ledger. */
...
/**
 * Generate a random 32-byte voter secret key. This is the *private witness*:
 * it stays in the local private-state store and is never written on-chain.
 * The contract only ever publishes a one-way hash (nullifier) of it.
 */
const freshVoterSecretKey = (): Uint8Array => Buffer.from(generateRandomSeed());

/**
 * Build a fresh private state for this voter. The 32-byte secret key is the
 * private witness — it is stored locally and never written to the ledger.
 */
export const freshVotingPrivateState = (): VotingPrivateState => createVotingPrivateState(freshVoterSecretKey());
...
export const getVotingLedgerState = async (
  ...
  const tally = await providers.publicDataProvider.queryContractState(contractAddress).then((contractState) => {
    if (contractState == null) return null;
    const l = Voting.ledger(contractState.data);
    return { yesVotes: l.yesVotes, noVotes: l.noVotes, totalVotes: l.yesVotes + l.noVotes };
  });
  logger.info(
    `Ledger state: ${tally != null ? JSON.stringify(tally, (_k, v) => (typeof v === 'bigint' ? v.toString() : v)) : 'null'}`,
  );
  return tally;
};

export const votingContractInstance: VotingContract = new Voting.Contract(witnesses);
...
export const joinContract = async (
  ...
  const votingContract = await findDeployedContract(providers, {
    ...
    initialPrivateState: createVotingPrivateState(freshVoterSecretKey()),
  });
...
export const vote = async (votingContract: DeployedVotingContract, choice: boolean): Promise<FinalizedTxData> => {
  logger.info(`Casting ${choice ? 'YES' : 'NO'} vote...`);
  const finalizedTxData = await votingContract.callTx.vote(choice);
  logger.info(`Transaction ${finalizedTxData.public.txId} added in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const displayVotingTally = async (
  providers: VotingProviders,
  votingContract: DeployedVotingContract,
): Promise<{ tally: VotingTally | null; contractAddress: string }> => {
  const contractAddress = votingContract.deployTxData.public.contractAddress;
  const tally = await getVotingLedgerState(providers, contractAddress);
  if (tally === null) {
    logger.info(`There is no voting contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Tally — YES: ${tally.yesVotes}  NO: ${tally.noVotes}  TOTAL: ${tally.totalVotes}`);
  }
  return { contractAddress, tally };
};
...
const printWalletSummary = (state: any, unshieldedKeystore: UnshieldedKeystore) => {
  ...
  const coinPubHex = typeof state.shielded.coinPublicKey === 'string' ? state.shielded.coinPublicKey : state.shielded.coinPublicKey.toHexString();
  const encPubHex = typeof state.shielded.encryptionPublicKey === 'string' ? state.shielded.encryptionPublicKey : state.shielded.encryptionPublicKey.toHexString();

  const coinPubKey = ShieldedCoinPublicKey.fromHexString(coinPubHex);
  const encPubKey = ShieldedEncryptionPublicKey.fromHexString(encPubHex);
  const shieldedAddress = ShieldedAddress.codec.encode(networkId, new ShieldedAddress(coinPubKey, encPubKey)).toString();

  const DIV = '──────────────────────────────────────────────────────────────';

  const dustAddressStr = state.dust?.address ? DustAddress.codec.encode(networkId, state.dust.address).toString() : 'n/a';
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<VotingCircuits>(contractConfig.zkConfigPath);
